...
`;
  ...
  try {
    // CONVERSATION_LOG_PATH is set by docker-compose to the shared volume path.
    // Falls back to local file when running without Docker.
    const logFilePath = process.env.CONVERSATION_LOG_PATH
      || path.join(__dirname, "conversation_log.txt");
